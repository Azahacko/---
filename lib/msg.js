...
const sms = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        // Correction ici : vérification de l'existence de m.msg et m.msg.contextInfo
        ...
        if (m.quoted) {
            ...
            if(quoted.viewOnceMessageV2) { 
                ...
                m.quoted.mtype = type
                // Correction ici : vérification de l'existence de m.msg et m.msg.contextInfo
                m.quoted.id = (m.msg && m.msg.contextInfo) ? m.msg.contextInfo.stanzaId : undefined
                m.quoted.chat = (m.msg && m.msg.contextInfo && m.msg.contextInfo.remoteJid) ? m.msg.contextInfo.remoteJid : m.chat
                m.quoted.isBot = m.quoted.id ? m.quoted.id.startsWith('BAES') && m.quoted.id.length === 16 : false
                m.quoted.isBaileys = m.quoted.id ? m.quoted.id.startsWith('BAE5') && m.quoted.id.length === 16 : false
                m.quoted.sender = (m.msg && m.msg.contextInfo && m.msg.contextInfo.participant) ? conn.decodeJid(m.msg.contextInfo.participant) : undefined
                m.quoted.fromMe = m.quoted.sender === (conn.user && conn.user.id)
                m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.conversation || m.quoted.contentText || m.quoted.selectedDisplayText || m.quoted.title || ''
                m.quoted.mentionedJid = (m.msg && m.msg.contextInfo) ? m.msg.contextInfo.mentionedJid : []
    ...
    m.text = (m.msg && (m.msg.text || m.msg.caption || m.msg.contentText || m.msg.selectedDisplayText || m.msg.title)) || m.message.conversation || ''
